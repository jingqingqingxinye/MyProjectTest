...
import com.example.mysdkforandroid.okhttp.widget.VideoFullDialog;
...
public class VideoAdSlot implements CustomVideoView.ADVideoPlayerListener {
  ...
  public VideoAdSlot(AdValue adInstance, AdSDKSlotListener slotListener, CustomVideoView.ADFrameImageLoadListener frameImageLoadListener) {
  ...
  private void initVideoView(CustomVideoView.ADFrameImageLoadListener frameImageLoadListener) {
    ...
    paddingView.setBackgroundColor(ContextCompat.getColor(mContext, R.color.black));
  ...
  public void updateAdInScrollView() {
    int currentArea = Utils.getVisiblePersent(mParentView);
    //小于0表示未出现在屏幕上，不做任何处理
    if (currentArea <= 0) {
      return;
    }
    ...
    if (currentArea < SDKConstant.VIDEO_SCREEN_PERCENT) {
      ...
      lastArea = 0;
      mVideoView.setIsComplete(false);//滑动出50%后标记为从头开始播放
  ...
  @Override public void onClickVideo() {
    ...
    if (mSlotListener != null) {
      if (mVideoView.isFrameHidden() && !TextUtils.isEmpty(desationUrl)) {
        ...
        ReportManager.pauseVideoReport(mXAdInstance.clickMonitor, mVideoView.getCurrentPosition() / SDKConstant.MILLION_UNIT);
      }
    } else {
      //走默认样式
      if (mVideoView.isFrameHidden() && !TextUtils.isEmpty(desationUrl)) {
