...
public class ResponseEntityToModule {
  ...
  private static void setFieldValue(Object moduleObj, JSONObject jsonObj, Class<?> clazz)
    ...
    Field[] fields = clazz.getDeclaredFields();//java反射机制， 获取本类中所有属性
    Class<?> cls;
    String name;
    for(Field f : fields) {
  ...
  private static void parseJsonArrayToRealmList(Field f, String name, Object moduleObj,
      JSONObject jsonObj) throws JSONException, IllegalAccessException, IllegalArgumentException {
    ...
    if (fc instanceof ParameterizedType) {
      ...
      if (pt.getActualTypeArguments()[0] instanceof Class) {
        ...
        if (jsonObj.get(name) instanceof JSONArray) {
          JSONArray array = jsonObj.getJSONArray(name);
          for (int i = 0; i < array.length(); i++) {
            if (array.get(i) instanceof  JSONObject) {
              objList.add(parseJsonArrayToRealmModel(array.getJSONObject(i), clss));
            }
          }
        }
      }
  ...
  public static RealmObject parseJsonArrayToRealmModel(JSONObject jsonObject, Class<?> clss) {
    RealmObject moduleObj = null;
    try {
      moduleObj = (RealmObject) clss.newInstance();
      setFieldValue(moduleObj, jsonObject, clss);
    } catch (IllegalArgumentException e) {
      e.printStackTrace();
    } catch (IllegalAccessException e) {
      e.printStackTrace();
  ...
  private static Object makeTypeSafeValue(Class<?> type, String value) throws NumberFormatException,
